<template>
    <div>
      <VUEConfirm :Popup="PopupContentAmm" :PathLogo="require('../../assets/images/LogoGemini2.png')"
      ...
      <VUEModal v-if="ConfermaDocumentiInviati" :PathLogo="require('../../assets/images/LogoGemini2.png')"
                                                :Programma="NomeProgramma"
                                                :Titolo="'Comunicazione'" :Altezza="'130px'" :Larghezza="'400px'"
      ...
      <div class="row" style="padding: 10px; border-top: 1px;">
        <div class="col-md-12">
          ...
          <button style="float:right;width:20%" class="btn btn-s-md btn-info" :disabled="ListaDocumentiSelezionati.length == 0 || AttivazioneInvioASdi" @click="OnClickInviaFattureSelezionate">Invia tutti i selezionati</button>
          <div style="float:right;width:1%">&nbsp;</div>
          <button style="float:right;width:20%" class="btn btn-s-md btn-info" :disabled="ListaDocumentiSelezionati.length == 0 || AttivazioneInvioASdi" @click="OnClickInviaFattureSelezionatePagina">Invia i selezionati della pagina</button>
          <div style="float:left;width:1%;">&nbsp;</div>
        </div>
...
methods :
{
  ...
  {
    var Self = this
    SystemInformation.AdvQuery.GetSQL('EsportaFatture',{},
                                      function(Results)
                                      {
  ...
  OnClickInviaFattureSelezionate()
  {
    if(!this.AttivazioneInvioASdi)
    {
      this.AttivazioneInvioASdi = true
      this.PopupContentAmm      = true
    }
  },

  OnClickInviaFattureSelezionatePagina()
  {
    if(!this.AttivazioneInvioASdi)
    {
      this.AttivazioneInvioASdi  = true
    ...
    get()
    {
      ...
      this.ListaDocumenti.forEach(function(Documento)
      {
        ...
        if(Documento.Selezionato && Documento.IS_NOTA_DI_CREDITO == 'T')
          Result.push({ChiaveNota : Documento.CHIAVE, IS_NOTA_DI_CREDITO : 'T', IS_AUTOFATTURA : 'F'})
        else 
        {
          if(Documento.Selezionato && Documento.IS_AUTOFATTURA == 'T')
            Result.push({ChiaveAutofattura : Documento.CHIAVE, IS_NOTA_DI_CREDITO : 'F', IS_AUTOFATTURA : 'T'})
          else 
          {
            if(Documento.Selezionato && Documento.IS_AUTOFATTURA == 'F' && Documento.IS_AUTOFATTURA == 'F')
              Result.push({ChiaveFattura : Documento.CHIAVE, IS_NOTA_DI_CREDITO : 'F', IS_AUTOFATTURA : 'F'})     
          }
        } 
      })
      return Result
