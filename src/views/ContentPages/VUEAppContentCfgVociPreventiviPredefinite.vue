<template>
  <VUEConfirm :Popup="PopupContentCfgVociPreventiviPredefinite" :Richiesta="'Annullare le modifiche effettuate?'" @onClickConfermaPopup="ConfermaElimina" @onClickChiudiPopup="AnnullaElimina">
  </VUEConfirm>
...
export default 
{
  ...
  data()
  {
    ...
    var Colonna = DataTable.AddColumn('Voci preventivi predefinite',
    ...
    return {
        ...
        PopupContentCfgVociPreventiviPredefinite  : false
    }
  },
  components: 
  {
  ...
  methods:
  {
      ...
      this.DataTable.Righe.forEach(function(Riga)
      {
          if(Riga.Nuovo)
             ObjQuery.Operazioni.push({
                                        Query     : "InserisciVocePreventivoPredefinita",
          ...
          else
          {
            if(Riga.Eliminato)
               ObjQuery.Operazioni.push({
                                          Query     : "EliminaVocePreventivoPredefinita",
            ...
            else 
            {
              if(Riga.Modificato())
                 ObjQuery.Operazioni.push({
                                            Query     : "ModificaVocePreventivoPredefinta",
                                            Parametri : Self.DataTable.PrepareQueryParameters(Riga)
                                          })
            }
          }
    ...
    AnnullaElimina()
    {
      this.PopupContentCfgVociPreventiviPredefinite = false
    },
    OnClickAnnulla()
    {
      this.PopupContentCfgVociPreventiviPredefinite = true
    },
    Annulla()
    {
      var Self = this;
      SystemInformation.AdvQuery.GetSQL(this.NomeModello,{},
                                        function(Results)
                                        {
                                          let ArrayInfo = SystemInformation.AdvQuery.FindResults(Results,"TutteLeVociPreventiviPredefinite");
