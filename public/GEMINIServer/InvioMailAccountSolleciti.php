<?php
  ...
  class TExtraInvioMail extends TAdvQueryClassForMail
  {
    protected function FExtraScriptServerSide($PDODBase, &$JSONAnswer)
    {
      ...
      {
        $this->GetParameterEmail($PDODBase, DEVELOPER_MODE? MAIL_ACCOUNT_AVVISI : MAIL_ACCOUNT_SOLLECITI);
    ...
    private function FGestioneInoltroEmail($Parametri, &$NomeCartella)
    {
      ...
      $NomeCartella = 'AttacInoltri' . gmdate('d_m_Y_H_i_s', (int)$Microtime) . "_$Milliseconds";
     ...
     {
       $LsChiaviFattureA = array();
       if(isset($Parametri->LsChiaviFattureA))
         $LsChiaviFattureA = $Parametri->LsChiaviFattureA;
      ...
      //  $NomeCartella = 'AttacFattPregresse_' . gmdate('d_m_Y_H_i_s', time());
       ...
       $PercorsoCartella = $this->FNomeCartellaAttacMail . '/' . $NomeCartella;
       if (!is_dir($PercorsoCartella))
       {
        
        //  $NomeCartella = 'Allegati_' . gmdate('Ymd_His', time());
         $Microtime = microtime(true);
         $Milliseconds = sprintf('%03d', ($Microtime - floor($Microtime)) * 1000);
         $NomeCartella = 'Allegati_' . gmdate('Ymd_His', (int)$Microtime) . "_$Milliseconds";

         $PercorsoCartella = $this->FNomeCartellaAttacMail . '/' . $NomeCartella;
         ForceDirectory($PercorsoCartella);
       }
       
       if (!empty($LsChiaviFattureA))  
       {
         $Parametri->Chiave   = $LsChiaviFattureA;
         $AConnection         = new TExtraStampaFattura($Parametri, false, true, true);
         $Base64StampaFattura = $AConnection->ServerSideScript(false);

         $Parametri->PDF     = $Base64StampaFattura->PDF;
         $FileContentFattura = $Parametri->PDF;
         $PDFdata            = base64_decode($FileContentFattura);

         $NomeFattura = 'Fatture_insolute_dal_2024.pdf';
         file_put_contents($PercorsoCartella . '/' . $NomeFattura, $PDFdata);
       }
         ...
         for ($i = 0; $i < count($InfoPosizione); $i++)
         {
           if (is_string($InfoPosizione[$i]))
           {
             ...
             if($NumeroFattura !== null && file_exists($FilePath))
             {
               ...
               $PercorsoCompleto = $PercorsoCartella . '/' . $NomeAllegato;
             ...
             else  error_log("File non trovato o fattura non valida: " . $FilePath);
           }
         }
       }
     }
  ...
  $AConnection = new TExtraInvioMail();
