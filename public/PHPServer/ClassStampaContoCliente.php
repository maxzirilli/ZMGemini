<?php       
  ...
  class TExtraStampaContoCliente extends TAdvQuery
  {
    ...
    private function FGetDatiStampa($Parametri, $PDODBase, &$JSONAnswer, $NomeLogo)
    {
      ...
      $this->FGestioneChiusureAnnuali($PDODBase, $DataDal, $DataAl, $ChiaveCliente);
      ...
      if($VelocizzaXStampaSaldiClienti)
      {
        foreach($this->FRigheProspetto as $Riga)
        {
          if($Riga->Tipo == TYP_APERTURA_ANNO)
          {
            if($PrimoElemento)
            {
              $DatiFooter->LB_TOTALE_SALDO   += $Riga->Importo;
              $DatiFooter->LB_TOTALE_SALDO = round($DatiFooter->LB_TOTALE_SALDO, 2); //Arrotondamento
            }
          }
          else
          {
            if($Riga->IsDare)
              $DatiFooter->LB_TOTALE_SALDO    += $Riga->Importo;
            else $DatiFooter->LB_TOTALE_SALDO -= $Riga->Importo;
      ...
      foreach($this->FRigheProspetto as $Riga)
      {
        ...
        if($Riga->Tipo == TYP_APERTURA_ANNO)
        {
          ...
          if($PrimoElemento)
          {
            ...
            $Fattura->LB_SALDO              = $DatiFooter->LB_TOTALE_SALDO;
          }
          else
          {
      ...
      $DatiFooter->TOTALE_SALDO    = $DatiFooter->LB_TOTALE_SALDO;
    ...
    private function FGestioneChiusureAnnuali($PDODBase, $DataDal, $DataAl, $ChiaveCliente)
    {
      ...
      if(count($ArrayPrimoMeseAnniIntervallo) != 0)
      {
        ...
        foreach($ArrayPrimoMeseAnniIntervallo as $OggettoAnno)
        {
          if(isset($OggettoAnno->CifraChiusura))
          {
            $this->FAddNewRiga('Chiave' . $OggettoAnno->Anno,
                               ...
                               $OggettoAnno->CifraChiusura, 
                               true);
          }
          else 
          {
            $CifraChiusuraAnnuale = $this->FCalcoloCifraChiusuraAnnuale($OggettoAnno->Anno, $ChiaveCliente, $PDODBase);
    ...
    private function FCalcoloCifraChiusuraAnnuale($OggettoAnno, $ChiaveCliente, $PDODBase)
    {
      $DataDal = "2000-01-01";
      $DataAl  = "$OggettoAnno-12-31";
      ...
      try
      {
        ...
        while($Row = $Query->fetch(PDO::FETCH_ASSOC))
        {
          ...
          if($Row['DATA'] >= $DataDal && $Row['DATA'] <= $DataAl)
            ...
            else $TotaleSaldoAnno -= $Row['IMPORTO'] / 100;
      ...
      { 
        ...
        { 
          ...
          $TotaleSaldoAnno += $TotaliFattura[0]->Totale;

          if(isset($Row['NUMERO_ANTICIPO']))
            $TotaleSaldoAnno -= $Row['NUMERO_ANTICIPO'] / 100;
        }
      }  
      else throw new Exception('Impossibile leggere le fatture attive dal database...');
      ...
      if($Query = $PDODBase->query($SQLBody))
      {
        ...
        { 
          ...
          $TotaleSaldoAnno -= $TotaliNota[0]->Totale;
        }
      }  
      else throw new Exception('Impossibile leggere le note del cliente dal database');
        ...
        { 
          ...
          if(isset($Row['DATA_PAGAMENTO_RATA']))
          {
            ...
            if (($TotaliNota[0]->TotaleRitenuta) != 0 && count($ArrayRate) > 0)
            {
              for($i = 0 ; $i < count($ArrayRate); $i++)
              {
                ...
                {
                   $TotaleSaldoAnno += $ArrayRate[$i]->Ritenuta;
                }
              }
            }
          }
          else
          {
            if(isset($Row['ID_FATTURA']))
            {
              ...
              $TotaleSaldoAnno -= $Row['IMPORTO'] / 100; // AGGIUNGO L'INCASSO DELLA FATTURA
              
              $TotaliFattura = TSystemInformation::GetRitenutaFatturaXRate($PDODBase, $Row['ID_FATTURA']);
              if($TotaliFattura[0]->TotaleRitenuta != 0)
                $TotaleSaldoAnno -= $this->FGetTotaleRitenutaRata($TotaliFattura[0], $Row['CHIAVE_RATA']);
        ...
        while($Row = $Query->fetch(PDO::FETCH_ASSOC))
        {
          ...
          if(isset($Row['IS_RITENUTA_ACCONTO']) && $Row['IS_RITENUTA_ACCONTO'] == 'T')
          {
            if(isset($Row['IS_NON_SCARICATA']) && $Row['IS_NON_SCARICATA'] != 'T') //SE LA RATA è UNA RDA BISOGNA CONTROLLARE SE DEVE ESSERE SCALATA DALLA FATTURA (PROBLEMA POSTE)
              $TotaleSaldoAnno -= TSystemInformation::GestioneArrotondamentoRateDerivatoDaErroreArrotondamento($Row['CHIAVE_RATA'], $TotaliFattura[0]);
          }
          else $TotaleSaldoAnno -= TSystemInformation::GestioneArrotondamentoRateDerivatoDaErroreArrotondamento($Row['CHIAVE_RATA'], $TotaliFattura[0]);
        ...
        while($Row = $Query->fetch(PDO::FETCH_ASSOC))
        {
          if(!isset($Row['ID_CLIENTE']))
          {
            ...
            if($Row['TIPO_MOVIMENTO'] == 'F')
            {
              ...
              TSystemInformation::GestioneArrotondamentoSaldiAnnualiDerivatoDaErroreArrotondamento($TotaleSaldoAnno, $Row['CHIAVE_RATA'], $TotaliFattura[0]);
      ...
      $SQLBody = "INSERT INTO saldi_chiusure_annuali (ID_CLIENTE, ANNO, CIFRA_CHIUSURA)
                                         VALUES ($ChiaveCliente, $OggettoAnno, $TotaleSaldoAnno * 100)";
