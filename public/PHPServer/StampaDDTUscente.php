      ...
      {            
        private function FGetDatiStampa($ChiaveDDT, $PDODBase, &$JSONAnswer, $NomeLogo)
        {
			   ...
			   $SQLBody = "SELECT ddt_uscenti.*, 
                ...
                $SQLBody = "SELECT voci_ddt_uscenti.*, unita_di_misura.DESCRIZIONE AS DESCRIZIONE_UNITA_DI_MISURA
                              FROM voci_ddt_uscenti
                              LEFT OUTER JOIN unita_di_misura ON unita_di_misura.CHIAVE = voci_ddt_uscenti.UNITA_DI_MISURA
                             WHERE voci_ddt_uscenti.ID_DDT = $ChiaveDDT
                             ORDER BY voci_ddt_uscenti.ORDINAMENTO";
